...
// Il parametro :id nel percorso viene automaticamente catturato e reso disponibile 
...
app.post('/users', (request, response) => {
    ...
    users.push(obj);
    response.json({message: 'Utente aggiunto con successo'})
})
...
app.put('/users/:id', (request, response) => {
    ...
    response.json({message: 'Utente modificato con successo'})
})

app.delete('/users/:id', (request, response) => {
    const id = request.params.id;   
    users = users.filter(u => u.id !== +id)
    response.json({message: 'Utente cancellato con successo'})
})
